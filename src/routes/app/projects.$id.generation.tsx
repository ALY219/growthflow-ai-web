...
import type { WebsiteBlueprint, GenerationError } from '@/lib/generation-types'
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {project ? `Generating blueprint for ${project.name}` : 'Generating website blueprint...'}
        </p>
      </div>

      <AnimatePresence mode="wait">
        {viewState === 'loading' && <LoadingView key="loading" />}
        {viewState === 'success' && blueprint && (
          <SuccessView key="success" blueprint={blueprint} projectId={id} />
        )}
        {viewState === 'error' && error && (
          <ErrorView key="error" error={error} onRetry={runGeneration} />
        )}
      </AnimatePresence>
    </div>
  )
        ...
        <CardContent className="p-12">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-md">
              Google Gemini AI is analyzing your business information and creating a complete website blueprint...
            </p>
      ...
      <Card className="border-border bg-gradient-to-r from-primary/10 to-accent/10">
        <CardContent className="p-8">
          <div className="flex items-start gap-4">
            <div className="flex size-12 items-center justify-center rounded-xl bg-primary shrink-0">
              <Sparkles className="size-6 text-primary-foreground" />
            </div>
            <div>
              <h2 className="text-2xl font-bold">{blueprint.siteName}</h2>
              {blueprint.tagline && (
                <p className="text-muted-foreground mt-1">{blueprint.tagline}</p>
              )}
            </div>
          </div>
        </CardContent>
        ...
        <Card className="border-border bg-card">
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              {Object.entries(blueprint.colorPalette).map(([key, color]) => (
                color && (
                  <div key={key} className="text-center">
                    <div
                      className="h-16 rounded-lg border border-border mb-2"
        ...
        <Card className="border-border bg-card">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold">Recommended Fonts</h3>
            </div>
            <div className="flex flex-wrap gap-2">
              {blueprint.recommendedFonts.map((font) => (
                <span key={font} className="rounded-full border border-border bg-accent px-3 py-1.5 text-sm font-medium">
                  {font}
                </span>
              ))}
            </div>
              ...
              {blueprint.sections.map((section, i) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <span className="flex size-6 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary shrink-0 mt-0.5">
                      {i + 1}
                    </span>
                    <div>
        ...
        <Card className="border-border bg-card">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold">Pages ({blueprint.pages.length})</h3>
            </div>
        ...
        <Card className="border-border bg-card">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold">Design Notes</h3>
            </div>
...
function ErrorView({ error, onRetry }: { error: GenerationError; onRetry: () => void }) {
  const errorLabels: Record<string, string> = {
    ...
    malformed_json: 'Malformed Response',
  ...
  return (
    ...
    >
      <Card className="border-destructive/30 bg-destructive/5">
        <CardContent className="p-8">
          <div className="flex flex-col items-center text-center">
            <div className="flex size-14 items-center justify-center rounded-xl bg-destructive/10 mb-4">
            ...
            <Button onClick={onRetry} className="mt-6 gap-2" size="lg">
