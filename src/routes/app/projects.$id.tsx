...
function ProjectDetailPage() {
  ...
  const handleWizardSubmit = useCallback(async () => {
    ...
    try {
      await createGenJob.mutateAsync({
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Card className="border-border bg-card">
        <CardContent className="p-8 text-center">
          <div className="inline-flex size-14 items-center justify-center rounded-xl bg-primary/10 mb-4">
            <Sparkles className="size-7 text-primary" />
          </div>
          <h2 className="text-lg font-semibold">Generate Your Website</h2>
          <p className="text-sm text-muted-foreground mt-1.5 max-w-md mx-auto mb-6">
            Use the AI wizard to configure your website and generate a complete blueprint with Google Gemini.
          </p>
